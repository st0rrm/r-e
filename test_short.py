    ...
    while simulation.getMinExpectedNumber() > 0:
        ...
        for vehicle_id in vehicle.getIDList():
            ...
            if vehicle.getParameter(vehicle_id, "state") == "":
                vehicle.setParameter(vehicle_id, "actualBatteryCapacity", random.randint(100,1000))
                ...
                if vehicle.getParameter(vehicle_id, "state") == "running":
                    ...
                    if vehicle.getParameter(vehicle_id, "next_charging_station") == "" and float(vehicle.getParameter(vehicle_id, "actualBatteryCapacity")) < simulation.findRoute(vehicle.getRoadID(vehicle_id),vehicle.getParameter(vehicle_id, "destination")).travelTime+25:
                        min_expect_time=math.inf #이거 무한으로 바꿔놔도 상관없긴 함.
        ...
        if sum(finish_info) >= 0.9 * destination_number*vehiclenum:
            np.save('r_vn_{}_{}_{}_traveltime_info'.format(velocity,destination_number,banbok),time_info)
            np.save('r_vn_{}_{}_{}_charge_info_{}'.format(velocity,destination_number,banbok,step), charge_info)
            traci.close()
            sys.stdout.flush()
        ...
        if step % 10 == 0:
            ...
            if namt > 1000:
                ...
                nam = namt
            print('현재 속도', velocity, banbok + 1, '번째', round(perc, 1), '% 진행중이며 시작한지',
                  int(round(time.time() - start_time, 0)), '초 경과되었으며 예상 종료 시간은', nam, '초 입니다.')
